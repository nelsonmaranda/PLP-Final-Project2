...
class ApiService {
  ...
  async signup(userData: SignupFormData): Promise<ApiResponse<AuthUser>> {
    try {
      const response: AxiosResponse<ApiResponse<AuthUser>> = await this.api.post('/auth/signup', userData)
      if (response.data.success && response.data.data) {
        localStorage.setItem('authToken', response.data.data.token)
        localStorage.setItem('user', JSON.stringify(response.data.data.user))
      }
  ...
  // Routes
  async getRoutes(): Promise<ApiResponse<RouteWithScores[]>> {
    try {
      const response: AxiosResponse<ApiResponse<RouteWithScores[]>> = await this.api.get('/routes')
      return response.data
    } catch (error) {
      throw this.handleError(error)
    }
  ...
  async getScores(): Promise<ApiResponse<Score[]>> {
    try {
      const response: AxiosResponse<ApiResponse<Score[]>> = await this.api.get('/scores')
      return response.data
    } catch (error) {
      throw this.handleError(error)
    }
  ...
  async getScoresByRoute(routeId: string): Promise<ApiResponse<Score[]>> {
    try {
      const response: AxiosResponse<ApiResponse<Score[]>> = await this.api.get(`/scores/route/${routeId}`)
      return response.data
    } catch (error) {
      throw this.handleError(error)
    }
  ...
  // Search
  async searchRoutes(query: string): Promise<ApiResponse<RouteWithScores[]>> {
    try {
      const response: AxiosResponse<ApiResponse<RouteWithScores[]>> = await this.api.get(
